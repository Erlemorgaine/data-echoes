export const useP5Layers = (p5) => {
  ...
  function beginLayer(graphicsOrKey, w, h, renderer) {
    ...
    let graphics =
      graphicsOrKey instanceof p5.Graphics ? graphicsOrKey : layerDict.get(graphicsOrKey)

    if (!graphics) {
      graphics = p5.createGraphics(w || width, h || height, renderer || p5.P2D)
